...
class Note:
    ...
    def fetch(cls, uuid):
        ...
        self.body = r[0]._mapping['body']
        self.byline = r[0]._mapping['byline']
        self.uuid = uuid
        return self
    ...
    def store(self):
        ...
        try:
            ...
            self.uuid = result.fetchone()._mapping['uuid']
...
class Inbox:
    ...
    def link_or_create(cls, auth_id, nickname, email):
        ...
        if row:
            existing_slug = row._mapping['slug']
            existing_email = row._mapping['email']
    ...
    def is_email_enabled(cls, slug):
        ...
        try:
            r = conn.execute(q, {"slug": slug}).fetchall()
            return bool(r[0]._mapping['email_enabled'])
    ...
    def get_email_template_name(cls, slug):
        ...
        r = conn.execute(q, {"slug": slug}).fetchall()
        return r[0]._mapping['email_template_name'] if r else 'default'
    ...
    def is_enabled(cls, slug):
        ...
        try:
            r = conn.execute(q, {"slug": slug}).fetchall()
            if not r[0]._mapping['enabled']:
                return False
            return bool(r[0]._mapping['enabled'])
        except InFailedSqlTransaction:
            print(traceback.print_exc())
            logging.error(traceback.print_exc())
            return False
    ...
    def search_notes(self, search_str, page, page_size):
        ...
        result = conn.execute(
            query,
            {"param": search_str_lower, "auth_id": self.auth_id, "limit": page_size, "offset": offset}
        ).fetchall()
        ...
        total_notes = result[0]._mapping['total_notes'] if result else 0
    ...
    def archived_notes(self):
        ...
        notes = [
            Note.from_inbox(self.slug, n._mapping['body'], n._mapping['byline'], n._mapping['archived'], n._mapping['uuid'])
            for n in r
        ]
        return notes[::-1]
